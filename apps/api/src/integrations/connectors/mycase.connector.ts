...
export class MyCaseConnector implements IncrementalSyncConnector {
  ...
  async exchangeAuthorizationCode(params: ConnectorTokenExchangeParams): Promise<ConnectorTokenExchangeResult> {
    ...
    if (!liveOauth) {
      return {
        accessToken: `mycase_access_${params.code.slice(0, 12)}`,
        refreshToken: `mycase_refresh_${params.code.slice(0, 12)}`,
    ...
    if (!response.ok) {
      ...
      throw new Error(`MyCase token exchange failed (${response.status}): ${text}`);
    }
    ...
    if (!payload.access_token) {
      throw new Error('MyCase token exchange did not return access_token');
    }
